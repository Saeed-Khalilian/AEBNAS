...
def get_net_info(net, data_shape, measure_latency=None, print_info=True, clean=False, lut=None, 
    ...
    return {
        ...
        'tiny_ml' : tiny_ml(params = params, flops = macs, activations = activations, 
                            pmax = pmax, fmax = fmax, amax = amax,
                            wp = wp, wf = wf, wa = wa, penalty = penalty),
        'gpu': gpu_latency, 'cpu': cpu_latency, 'util': 1, 'avg_macs': macs
    }
...
class OFAEvaluator:
    ...
    def __init__(self,
                 ...
                 pretrained = False,
                 kernel_size=None, exp_ratio=None, depth=None, threshold = None, ks_exit = None,  input_channel = None, target_macs = 2.44, alpha_macs = 5.0):
        ...
        if ('ofa_mbv3_d234_e346_k357_w1.0' in model_path) or ('ofa_mbv3_d234_e346_k357_w1.2' in model_path):
            ...
            self.e_ks = [[[0, 4], [3, 1], [3, 4], [0, 2]], [[3, 1], [3, 3], [3, 3], [5, 3]], [[0, 1], [0, 1], [3, 1], [3, 3]], [[1, 1], [1, 4], [1, 3], [5, 3]]] if ks_exit is None else ks_exit
            #TODO: allready pass here
            self.engine = OFAEEMobileNetV3(
                n_classes=n_classes,
                dropout_rate=0, width_mult_list=self.width_mult, ks_list=self.kernel_size,
                expand_ratio_list=self.exp_ratio, depth_list=self.depth, e_ks = self.e_ks, input_channel = self.input_channel)
    ...
    def sample(self, config=None):
        ...
        if config is not None:
            if isinstance(self.engine,OFAEEMobileNetV3):
              self.engine.set_active_subnet(ks=config['ks'], e=config['e'], d=config['d'], t=config['t'], e_ks = config['e_ks'])
            else:
    ...
    def save_net_config(path, net, config_name='net.config'):
        ...
        if isinstance(net, torch.nn.DataParallel):
          json.dump(net.module.config, open(net_save_path, 'w'), indent=4)
        else:
    ...
    @staticmethod   
    def adaptive_eval(subnet_dic, subnet, data_path, dataset='imagenet', n_epochs=0, resolution=(224,224), input_channel=3, trn_batch_size=128, vld_batch_size=250,
             num_workers=4, valid_size=None, is_test=True, log_dir='.tmp/eval', measure_latency=None, no_logs=False,
             reset_running_statistics=True, pmax = 2, fmax = 100, amax = 5, wp = 1, wf = 1/40, wa = 1, penalty = 10**10,save_subnet="", save_stats = "", threshold=None, target_macs = 2.44, alpha_macs = 5.0):
        ...
        info = get_adapt_net_info(
              subnet, (input_channel, resolution, resolution), measure_latency=measure_latency,
              print_info=False, clean=True, lut=lut, pmax = pmax, fmax = fmax, amax = amax, wp = wp, wf = wf, wa = wa, penalty = penalty)
        ...
        run_manager.info = info
        run_manager.threhold_range = threshold 
        run_manager.target_macs = target_macs
        run_manager.alpha_macs = alpha_macs 
        run_manager.input_shape = (input_channel, resolution, resolution)
        ...
        #updating net.subnets file by the new thresholds
        if save_subnet != "":
            with open(save_subnet, 'r') as f:
                stats = json.load(f)
            if isinstance(run_manager.net, torch.nn.DataParallel):
               stats['t'] = run_manager.net.module.threshold
            else:
                stats['t'] = run_manager.net.threshold
            with open(save_subnet, 'w') as f:
                json.dump(stats, f)
            print(run_manager.info,  stats['t'])
        ...
        subnet = subnet.cuda() 
        subnet.eval()
        info2 = get_adapt_net_info(subnet, (input_channel, resolution, resolution), measure_latency=measure_latency,print_info=False, clean=True, lut=lut, pmax = pmax, fmax = fmax, amax = amax, wp = wp, wf = wf, wa = wa, penalty = penalty)

        info['macs'] = info2['macs']
...
def main(args):
    ...
    if args.threshold is not None:
        s=args.threshold
        s = s.strip("[]")   
        threshold = [float(x) for x in s.split(",")]
        print(threshold)  # Output: [0.1, 0.2, 1.0]
    ...
    if mode == 'config':
        ...
        evaluator = OFAEvaluator(n_classes=args.n_classes, model_path=args.supernet, pretrained = args.pretrained, input_channel = args.input_channel, threshold=threshold, target_macs=args.target_macs, alpha_macs=args.alpha_macs)
        if ('w1.0' in args.supernet):
          subnet, _ = evaluator.sample({'ks': config['ks'], 'e': config['e'], 'd': config['d']})
        if ('eembv3' in args.supernet):
          subnet, _ = evaluator.sample({'ks': config['ks'], 'e': config['e'], 'd': config['d'], 't':config['t'], 'e_ks':config['e_ks']})
        resolution = config['r']
    ...
    if ('eembv3' in args.supernet):
        OFAEvaluator.adaptive_eval(args.subnet,
            subnet, log_dir=args.log_dir, data_path=args.data, dataset=args.dataset, n_epochs=args.n_epochs,
            resolution=resolution, input_channel=args.input_channel, trn_batch_size=args.trn_batch_size, vld_batch_size=args.vld_batch_size,
            num_workers=args.num_workers, valid_size=args.valid_size, is_test=args.test, measure_latency=args.latency,
            no_logs=(not args.verbose), reset_running_statistics=args.reset_running_statistics, 
            pmax = args.pmax, fmax = args.fmax, amax = args.amax, wp = args.wp, wf = args.wf, wa = args.wa, penalty = args.penalty, save_subnet=args.subnet, save_stats= args.save
            ,threshold=threshold, target_macs=args.target_macs, alpha_macs=args.alpha_macs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--threshold', type=str, default=None, help='input threshold as a string')
    parser.add_argument('--target_macs', type = float, default=2.44, help='target_macs')
    parser.add_argument('--alpha_macs', type = float, default=5.0, help='alpha_macs')
