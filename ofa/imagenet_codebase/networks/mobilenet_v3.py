...
# from layers import *
from ofa.layers import set_layer_from_config, MBInvertedConvLayer, ConvLayer, IdentityLayer, LinearLayer, PoolingLayer
...
from ofa.elastic_nn.modules.dynamic_layers import ExitBlock, ExitBlock_Modified

class EEMobileNetV3(MyNetwork):

    def __init__(self, first_conv, blocks, final_expand_layer, feature_mix_layer, classifier, 
    n_classes, dropout_rate, d_list, t, e_ks):

        super(EEMobileNetV3, self).__init__()

        #print('-----------------------------EE MobileNetV3----------------------------------------')
        self.b_temp = blocks
        ...
        for i in range(0,n_blocks-1,1):
            ...
            if (t[i]!=1):
                ...
                exit_list.append(ExitBlock_Modified(self.n_classes,final_expand_width,feature_dim,last_channel,self.dropout_rate,self.e_ks[i]))
        self.n_exit = len(exit_list)
        self.t_list = t_list
        self.exit_idxs = exit_idxs
        self.exit_list = nn.ModuleList(exit_list)
        self.total_number_of_exits = len(t)
        # print("Number of exits:", self.n_exit, self.exit_idxs, self.total_number_of_exits, self.t_list, t )

        self.inference = False #inference without considering the thresholds
    
    def forward(self, x):
        self.channels = []
        self.e_in = []

        self.channels.append(x.size())
        x = self.first_conv(x)
        ...
        if(self.training): #training 
            ...
            # inference without considering the thresholds 
            if self.inference:
                y = x.clone()
                i = 0
                for idx,block in enumerate(self.blocks):
                    if(self.n_exit!=0):
                        if (idx==self.exit_idxs[i]): #exit block
                            pred, _ = self.exit_list[i](y)
                            preds_inference.append(pred)
                            if i<(self.n_exit-1):
                                i+=1
                    y = block(y)
                y = self.final_expand_layer(y)
                y = y.mean(3, keepdim=True).mean(2, keepdim=True)  # global average pooling
                y = self.feature_mix_layer(y)
                y = torch.squeeze(y)
                y = self.classifier(y)
                preds_inference.append(y)
                return preds_inference
            ...
            for idx,block in enumerate(self.blocks):
                if(self.n_exit!=0):
                    if (idx==self.exit_idxs[i]): #exit block
                            ...
                            #the function below was added as a replacement of: p = np.where(np.array(mask)==False)[0]
                            p = np.where(np.array(mask)==False)[0]
                            """
                            p = []
                           
                            if(len(list(mask.size())) > 0):
                             
                              indices = []
                              mask_array = np.array(mask)

                              for j in range(len(mask_array)):
                                if mask_array[j] == False:
                                  indices.append(j)
    ...
    def set_inference(self, inference):
        """
        Set the inference mode for the network.
        If True, the network will not consider the thresholds during inference.
        """
        self.inference = inference

    def set_thresholds(self, thresh):
        """
        Set the thresholds for the exit blocks.
        :param t: List of thresholds for each exit block.
        """

        print("\n   initialized_thresholds:", self.initialized_thresholds, sum(1 for v in self.initialized_thresholds if v != 1))
        print("   threshold             :",self.t_list,  sum(1 for v in  self.t_list if v != 1) , self.n_exit)
        # print(self.exit_idxs, self.d_list)
        self.n_exit = sum(1 for v in thresh if v != 1)

        # print(type(thresh), type(self.t_list ),len(self.t_list),self.t_list)
        disabled=0
        for i in range(len(thresh)):
            # print(i,disabled )
            if self.initialized_thresholds[i] == 1 :
                disabled+=1
                continue
            if thresh[i] == 1:
                self.t_list.pop(i-disabled)
                self.exit_idxs.pop(i-disabled)
                self.exit_list.pop(i-disabled)
                disabled += 1
            else:
                self.t_list[i-disabled]=thresh[i]

                 
            self.threshold[i] = thresh[i]
    
        print("   updated_thresholds:",self.threshold, self.t_list, self.n_exit)
