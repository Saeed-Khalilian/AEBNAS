...
def plot_models(csv_paths, target_macs=None):
    ...
    for i, csv_path in enumerate(csv_paths):
        ...
        ax.scatter(pareto_df['avg_macs'], pareto_df['top1_acc'], 
                   c='red', marker='*', s=250, edgecolors='black', 
                   label='Pareto Front', zorder=5)
                   
        pareto_sorted = pareto_df.sort_values(by='avg_macs')
        if n > 1:
            md_lines.append(f"### {os.path.basename(csv_path)}\n")
        
        md_lines.append("```text")
        md_lines.append("")
        md_lines.append("="*50)
        md_lines.append(f"Pareto Front (Non-Dominated Architectures): {len(pareto_sorted)}")
        md_lines.append("="*50)
        md_lines.append(f"{'Top-1 Accuracy (%)':<20} | {'Average MACs':<15} | {'Iteration':<10}")
        md_lines.append("-" * 53)
        for _, r in pareto_sorted.iterrows():
            md_lines.append(f"{r['top1_acc']:<20.2f} | {r['avg_macs']:<15.2f} | {int(r['iteration']):<10}")
        md_lines.append("```\n")
        ...
        ax.set_xlabel("Average Number of MACs (M)")
        if i == 0:
        ...
        if target_macs and i < len(target_macs):
            ax.axvline(x=target_macs[i], color='red', linestyle='--', linewidth=2, label='Target MACs', zorder=3)
    ...
    # Save the figure and markdown
    if n == 1:
        out_path = os.path.join(script_dir, "models_found.png")
        md_out_path = os.path.join(script_dir, "models_found.md")
    else:
        out_path = os.path.join(script_dir, "comparison_models_found.png")
        md_out_path = os.path.join(script_dir, "comparison_models_found.md")
    ...
    with open(md_out_path, 'w') as f:
        f.write("\n".join(md_lines) + "\n")
    print(f"Markdown table saved to: {md_out_path}")
