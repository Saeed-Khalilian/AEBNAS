...
class GNN_Surrogate_Split(nn.Module):
    def __init__(self, num_node_features=18, hidden_dim=32, output_dimension=32,
        ...
        super(GNN_Surrogate_Split, self).__init__()
    ...
    def forward(self, x, edge_index, batch, input_resolution):
        ...
        predicted_accuracy = self.final_predictor(x)
...
class GIN_Split:
    ...
    def __init__(self, arch_encoder_kwargs=None, lr=3e-4, epochs=500, batch_size=16, weight_decay=0.0, **kwargs):
        ...
        self.model = GNN_Surrogate_Split(**kwargs)
        self.name = 'gin_split'
    ...
    def fit(self, x, y, **kwargs):
        ...
        self.model  = train(self.model, train_graphs, device=device, **self.train_kwargs)
...
def train_one_epoch(net, loader, criterion, optimizer, device):
    ...
    for batch in loader:
        ...
        pred = net(batch.x, batch.edge_index, batch.batch, batch.input_resolution)
...
def validate(net, loader, device):
    ...
    with torch.no_grad():
        ...
        for batch in loader:
            ...
            target = batch.y.view_as(pred) #* target_std.to(device) + target_mean.to(device)
